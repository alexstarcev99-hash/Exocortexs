...
**Статус:** На паузе — карьерный трек сместился на ML-инженера, см. [ml-engineer-roadmap.md](ml-engineer-roadmap.md). Прогресс этого файла (Python/API, неделя 1) перенесён туда как база. Этот roadmap — про работу с LLM через API (RAG, агенты), не про классический ML; может вернуться как специализация в Фазе 4 нового трека или отдельным проектом позже.
